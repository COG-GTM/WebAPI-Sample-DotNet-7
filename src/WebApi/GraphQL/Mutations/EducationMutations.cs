...
public class EducationMutations
{
        ...
        [Service] IEducationService educationService)
    {
        ...
        {
            return new EducationPayload(new Types.Error(ex.Message, "CREATE_FAILED"));
        }
    }
        ...
        [Service] IEducationService educationService)
    {
        try
        {
            ...
            if (!success)
            {
                return new EducationPayload(new Types.Error("Education record not found or update failed", "UPDATE_FAILED"));
            }
        ...
        {
            return new EducationPayload(new Types.Error(ex.Message, "UPDATE_FAILED"));
        }
    }
        ...
        [Service] IEducationService educationService)
    {
        try
        {
            ...
            if (!success)
            {
                return new DeleteEducationPayload(false, new Types.Error("Education record not found", "NOT_FOUND"));
            }
            return new DeleteEducationPayload(true);
        }
        catch (Exception ex)
        {
            return new DeleteEducationPayload(false, new Types.Error(ex.Message, "DELETE_FAILED"));
        }
    }
}
